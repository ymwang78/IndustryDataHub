...
class TestDADataSource(unittest.TestCase):
    ...
    def test_da_source(self):
        ...
        self.assertGreaterEqual(subscribe_result, IDH_ERRCODE.IDH_ERRCODE_SUCCESS.value, "Failed to subscribe tags")
        group_read_result, group_values = self.idh.read_group_values(group, handles)
        self.assertGreaterEqual(group_read_result, IDH_ERRCODE.IDH_ERRCODE_SUCCESS.value, "Failed to read group values")
        ...
        self.assertGreaterEqual(group_write_result, IDH_ERRCODE.IDH_ERRCODE_SUCCESS.value, "Failed to write group values")
